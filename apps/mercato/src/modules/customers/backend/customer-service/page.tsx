...
export default function CustomerServiceSettingsPage() {
  ...
  // null = watch all connected mailboxes. An array = only those ids; [] = none.
  ...
  const [newScenarioAction, setNewScenarioAction] =
    useState<FlagAction>('pause')
  ...
  async function reloadConnections() {
    const [allRes, csRes] = await Promise.all([
      fetch('/api/email/connections', { credentials: 'include' })
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/email/connections?purpose=customer_service', {
        credentials: 'include',
      })
        .then((r) => r.json())
        .catch(() => null),
  ...
  async function connectSupportInbox() {
    ...
    if (!supportEmail || !supportPassword) {
      setSupportError('Enter the support email address and its App Password.')
      return
    }
    ...
    try {
      ...
      if (supportShowAdvanced) {
        if (supportImapHost) {
          body.imapHost = supportImapHost
          body.imapPort = Number(supportImapPort)
        }
        if (supportSmtpHost) {
          body.smtpHost = supportSmtpHost
          body.smtpPort = Number(supportSmtpPort)
        }
      }
      ...
      if (data.ok) {
        ...
        setSupportPassword('')
        setSupportImapHost('')
        setSupportImapPort('993')
        setSupportSmtpHost('')
        setSupportSmtpPort('587')
        ...
        const newConn =
          (fresh?.cs || []).find(
            (c: EmailConnection) => c.email_address === connectedEmail,
          ) ||
          (fresh?.cs || []).find(
            (c: EmailConnection) => !csInboxes.some((x) => x.id === c.id),
          )
        if (newConn) {
          setWatchedIds((prev) => {
            const next =
  ...
  async function disconnectSupportInbox(id: string, address: string) {
    if (
      !confirm(
        `Disconnect ${address}? Customer Service will stop watching this support inbox.`,
      )
    )
      return
    setCsDisconnecting(id)
    try {
      await fetch(`/api/email/smtp?id=${id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      ...
      setWatchedIds((prev) =>
        prev === null ? prev : prev.filter((x) => x !== id),
      )
    } catch {}
  ...
  const [docProgress, setDocProgress] = useState<
    { name: string; status: 'pending' | 'done' | 'failed'; error?: string }[]
  >([])
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([
      fetch('/api/customer-service/settings', { credentials: 'include' })
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/email/connections', { credentials: 'include' })
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/customer-service/knowledge', { credentials: 'include' })
        .then((r) => r.json())
        .catch(() => null),
      fetch('/api/email/connections?purpose=customer_service', {
        credentials: 'include',
      ...
      if (settingsRes?.ok && settingsRes.data) {
        ...
        setWatchedIds(
          Array.isArray(s.watchedConnectionIds) ? s.watchedConnectionIds : null,
        )
        setReplyMode(
          s.replyMode === 'auto' || s.replyMode === 'hybrid'
            ? s.replyMode
            : 'draft',
        )
        if (
          typeof s.hybridConfidenceThreshold === 'number' &&
          Number.isFinite(s.hybridConfidenceThreshold)
        ) {
          setHybridThreshold(
            Math.min(1, Math.max(0, s.hybridConfidenceThreshold)),
          )
        }
      ...
      if (twilioRes?.ok && twilioRes.data?.phoneNumber)
        setTwilioNumber(twilioRes.data.phoneNumber)
  ...
  async function addModelAnswer() {
    setKbError('')
    if (!maContent.trim()) {
      setKbError('Enter the answer text first.')
      return
    }
    setKbSaving(true)
    try {
      const res = await fetch('/api/customer-service/knowledge', {
        ...
        body: JSON.stringify({
          kind: 'model_answer',
          title: maTitle.trim() || undefined,
          content: maContent.trim(),
        }),
      })
      const data = await res.json()
      if (data.ok) {
  ...
  async function addDocument() {
    setKbError('')
    setDocSummary('')
    if (docFiles.length === 0 && !docContent.trim()) {
      setKbError('Upload one or more files or paste the document text.')
      return
    }
    setKbSaving(true)
    try {
      if (docFiles.length > 0) {
        ...
        setDocProgress(
          docFiles.map((f) => ({ name: f.name, status: 'pending' as const })),
        )
        let added = 0
        let skipped = 0
        for (let i = 0; i < docFiles.length; i++) {
          ...
          try {
            ...
            if (data.ok) {
              ...
              setDocProgress((prev) =>
                prev.map((p, idx) =>
                  idx === i ? { ...p, status: 'failed', error: data.error } : p,
                ),
              )
            }
          } catch {
  ...
  async function addWebPage() {
    ...
    if (!url) {
      setUrlError('Enter a web page URL.')
      return
    }
    setUrlSaving(true)
    try {
  ...
  async function importSelectedKbDocs() {
    ...
    if (ids.length === 0) {
      setKbError('Select at least one document.')
      return
    }
    setKbImporting(true)
    try {
      ...
      if (data.ok) {
        setDocSummary(
          `${data.added ?? 0} added${data.skipped ? `, ${data.skipped} skipped` : ''} from Knowledge Base.`,
        )
  ...
  async function deleteKnowledge(id: string) {
    ...
    try {
      const res = await fetch(`/api/customer-service/knowledge/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      const data = await res.json()
      if (!data.ok) {
        setKnowledge(prev)
        setKbError(data.error || 'Failed to delete.')
      }
    } catch {
      setKnowledge(prev)
      setKbError('Failed to delete.')
    }
  }
  ...
  function generateCustomKey(label: string): string {
    ...
    return flagScenarios.some((s) => s.key === base)
    ...
    const res = await fetch('/api/customer-service/settings', {
      ...
      body: JSON.stringify({
        watchedConnectionIds:
          overrides && 'watchedConnectionIds' in overrides
            ? overrides.watchedConnectionIds
            : watchedIds,
        replyMode,
  ...
  const personalMailboxes = connections.filter(
    (c) => c.purpose !== 'customer_service',
  )
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-1">
        <h1 className="text-lg font-semibold flex items-center gap-2">
          <Headphones className="size-5 text-muted-foreground" /> Customer
          Service
        </h1>
      </div>
      <p className="text-sm text-muted-foreground mb-4">
        Let Noli reply to incoming customer emails. Choose whether replies wait
        for your approval, send automatically, or send only when they are
        confident and safe.
      </p>

      <Tabs
        value={tab}
        onValueChange={(v) => setTab(v as 'queue' | 'settings' | 'accounts')}
      >
            ...
            <>
              ...
              <section className="mb-8">
                <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                  <FileEdit className="size-4 text-muted-foreground" /> Reply
                  mode
                </h2>
                ...
                {replyMode === 'hybrid' && (
                  <div className="mt-3 rounded-lg border px-4 py-3">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (Number.isFinite(v))
                            setHybridThreshold(Math.min(1, Math.max(0, v)))
                        }}
                    ...
                    <p className="text-[12.5px] text-muted-foreground mt-2">
                      A reply auto-sends only when Noli is at least this
                      confident in its answer and judges it safe to send.
                      Everything else waits for your approval. Higher values
                      send fewer replies on their own.
                    </p>
                  </div>
                )}
              </section>
              ...
              <section className="mb-8">
                <h2 className="text-sm font-semibold mb-1 flex items-center gap-2">
                  <Flag className="size-4 text-muted-foreground" /> Flag
                  scenarios
                </h2>
                <p className="text-xs text-muted-foreground mb-3">
                  Tell Noli which situations to watch for. When an incoming
                  message matches an enabled scenario, Noli flags it, drafts a
                  reply using your instructions, and emails you an alert. Pause
                  for review holds the reply in your queue, even in auto-send
                  mode. Auto-send lets the reply go out on its own.
                </p>
                <div className="rounded-lg border divide-y">
                          ...
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <select
                                ...
                                onChange={(e) =>
                                  updateFlagScenario(s.key, {
                                    action: e.target.value as FlagAction,
                                  })
                                }
                                disabled={!s.enabled}
                              ...
                              {custom && (
                                <button
                                  type="button"
                                  onClick={() => removeCustomScenario(s.key)}
                                  className="shrink-0 text-muted-foreground hover:text-[#b91c1c] transition p-1"
                ...
                <div className="rounded-lg border mt-3">
                  <div className="px-4 py-3 border-b">
                    <p className="text-sm font-medium flex items-center gap-2">
                      <Plus className="size-4 text-muted-foreground" /> Add a
                      custom scenario
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Define your own situation to watch for, such as a
                      wholesale inquiry or a press request. Noli flags matching
                      messages and follows your instructions.
                    </p>
                  </div>
                  <div className="px-4 py-3 space-y-3">
                    ...
                    />
                    <textarea
                      value={newScenarioInstructions}
                      onChange={(e) =>
                        setNewScenarioInstructions(e.target.value)
                      }
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <select
                        ...
                        onChange={(e) =>
                          setNewScenarioAction(e.target.value as FlagAction)
                        }
                ...
                <div className="rounded-lg border">
                  <div className="px-4 py-3">
                    ...
                    <p className="text-[12.5px] text-muted-foreground mt-1.5">
                      Added to the end of drafted replies. We prefilled a
                      default from your business name. Edit or clear it anytime.
                      You can still edit each draft before sending.
                    </p>
                  </div>
                </div>
              </section>
              ...
              <div className="flex items-center gap-2 mb-10 h-5 text-xs">
                {saving ? (
                  ...
                  <span className="text-[#b91c1c] dark:text-[#f87171]">
                    {error} We will retry when you make another change.
                  </span>
                ) : saved ? (
                  <span className="text-[#047857] dark:text-[#34d399] flex items-center gap-1">
              ...
              <section className="mb-8">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Give Noli example answers and reference documents to draw
                  from. Drafted replies will reuse and adapt them when relevant.
                </p>
                ...
                <div className="rounded-lg border divide-y mb-4">
                  {knowledge.length === 0 ? (
                    <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                      No entries yet. Add a model answer or a reference document
                      below.
                    </div>
                  ) : (
                    knowledge.map((entry) => (
                          ...
                          <div className="flex items-center gap-2 mb-0.5">
                            <Badge variant="secondary" className="shrink-0">
                              {entry.kind === 'model_answer' ? (
                                ...
                                <span className="flex items-center gap-1">
                                  <Globe className="size-3" /> Web page
                                </span>
                              ) : (
                                <span className="flex items-center gap-1">
                ...
                <div className="rounded-lg border mb-4">
                  <div className="px-4 py-3 border-b flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Pull documents you have stored in your Knowledge Base
                        straight into this library.
                      </p>
                    </div>
                    {!kbPickerOpen && (
                      <Button
                        ...
                        className="shrink-0"
                      >
                        <BookOpen className="size-3.5 mr-1" /> Browse Knowledge
                        Base
                      </Button>
                    )}
                  </div>
                    ...
                    <div className="px-4 py-3 space-y-3">
                      {kbDocsLoading ? (
                        ...
                        <div className="rounded-md border px-4 py-6 text-center text-xs text-muted-foreground">
                          Could not connect to your Knowledge Base right now.
                          Make sure you have one set up, then try again.
                        </div>
                      ) : kbDocs.length === 0 ? (
                        <div className="rounded-md border px-4 py-6 text-center text-xs text-muted-foreground">
                        ...
                        <div className="max-h-64 overflow-y-auto rounded-md border divide-y">
                          {kbDocs.map((doc) => (
                            <label
                              key={doc.id}
                              className={`flex items-center gap-2 px-3 py-2 text-sm ${doc.alreadyImported ? 'opacity-60' : 'cursor-pointer hover:bg-muted/40'}`}
                            >
                              <input
                                type="checkbox"
                                disabled={doc.alreadyImported}
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          type="button"
                          size="sm"
                          onClick={importSelectedKbDocs}
                          disabled={kbImporting || kbSelectedIds.size === 0}
                        >
                          <Plus className="size-3.5 mr-1" />{' '}
                          {kbSelectedIds.size > 0
                            ? `Add selected (${kbSelectedIds.size})`
                            : 'Add selected'}
                        </Button>
                        <Button
                          type="button"
                ...
                <div className="rounded-lg border mb-4">
                  <div className="px-4 py-3 border-b">
                    <p className="text-sm font-medium flex items-center gap-2">
                      <Globe className="size-4 text-muted-foreground" /> Add a
                      web page
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Add your FAQ page or website URL. We pull the text to help
                  ...
                  <div className="px-4 py-3 space-y-3">
                    ...
                    {urlSuccess && (
                      <p className="text-xs text-[#047857] dark:text-[#34d399] flex items-center gap-1">
                        <Check className="size-3" /> {urlSuccess}
                      </p>
                    )}
                    <input
                      value={urlValue}
                    ...
                    >
                      {urlSaving ? (
                        ...
                        <>
                          <Plus className="size-3.5 mr-1" /> Add web page
                        </>
                      )}
                    </Button>
                  </div>
                ...
                <div className="rounded-lg border mb-4">
                  <div className="px-4 py-3 border-b">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      An example reply Noli can reuse or adapt for similar
                      questions.
                    </p>
                  </div>
                  <div className="px-4 py-3 space-y-3">
                    <input
                      value={maTitle}
                      onChange={(e) => setMaTitle(e.target.value)}
                ...
                <div className="rounded-lg border mb-4">
                  <div className="px-4 py-3 border-b">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Upload one or more PDF, Word (.docx), or text (.txt, .md,
                      .csv) files, or paste text. The text is pulled out
                      automatically. Each file becomes its own entry.
                    </p>
                  </div>
                  <div className="px-4 py-3 space-y-3">
                    <input
                    ...
                    {docFiles.length > 1 && (
                      <p className="text-[12.5px] text-muted-foreground">
                        {docFiles.length} files selected. Each is added as a
                        separate entry.
                      </p>
                    )}
                        ...
                        {docProgress.map((p, i) => (
                          ...
                          >
                            ...
                            {p.status === 'failed' && p.error && (
                              <span className="text-[#b91c1c] dark:text-[#f87171] truncate">
                                {p.error}
                              </span>
                            )}
                          </li>
                    ...
                    {docSummary && (
                      <p className="text-[12.5px] text-[#047857] dark:text-[#34d399]">
                        {docSummary}
                      </p>
                    )}
                    <p className="text-[12.5px] text-muted-foreground">
        ...
        <TabsContent value="accounts">
          {loading ? (
            ...
            <>
              ...
              <section className="mb-8">
                <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                  <Mail className="size-4 text-muted-foreground" /> Dedicated
                  support inbox
                </h2>
                <p className="text-xs text-muted-foreground mb-3">
                  Connect a mailbox used only for customer support, such as
                  support@yourbusiness.com. It stays separate from your personal
                  Inbox. Customer Service watches it on its own.
                </p>
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Dedicated to Customer Service (IMAP/SMTP)
                            </p>
                          </div>
                        </div>
                        <Button
                          ...
                          onClick={() =>
                            disconnectSupportInbox(conn.id, conn.email_address)
                          }
                        >
                          {csDisconnecting === conn.id ? (
                            'Disconnecting...'
                          ) : (
                            <>
                ...
                <div className="rounded-lg border">
                  <div className="px-4 py-3 border-b">
                    <p className="text-sm font-medium flex items-center gap-2">
                      ...
                      Connect a dedicated support inbox
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Works with Gmail, Outlook, or any provider that supports
                      IMAP/SMTP. Use an App Password, not your normal password.
                    </p>
                  </div>
                  <div className="px-4 py-3 space-y-2">
                    ...
                    {supportSuccess && (
                      <p className="text-xs text-[#047857] dark:text-[#34d399] flex items-center gap-1">
                        <Check className="size-3" /> Support inbox connected.
                      </p>
                    )}
                    ...
                    {supportShowAdvanced && (
                      <div className="grid grid-cols-2 gap-2 p-3 rounded-md bg-muted/40 border">
                        <Input
                          value={supportImapHost}
                          onChange={(e) => setSupportImapHost(e.target.value)}
                          placeholder="IMAP host (auto-detected)"
                          className="h-8 text-xs"
                        />
                        <Input
                          value={supportImapPort}
                          onChange={(e) => setSupportImapPort(e.target.value)}
                          placeholder="IMAP port (993)"
                          className="h-8 text-xs"
                        />
                        <Input
                          value={supportSmtpHost}
                          onChange={(e) => setSupportSmtpHost(e.target.value)}
                          placeholder="SMTP host (auto-detected)"
                          className="h-8 text-xs"
                        />
                        <Input
                          value={supportSmtpPort}
                          onChange={(e) => setSupportSmtpPort(e.target.value)}
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={connectSupportInbox}
                      disabled={
                        supportSaving || !supportEmail || !supportPassword
                      }
                    >
                      {supportSaving
                        ? 'Testing connection...'
              ...
              <section className="mb-8">
                <h2 className="text-sm font-semibold mb-3 flex items-center gap-2">
                  <Mail className="size-4 text-muted-foreground" /> Mailboxes to
                  watch
                </h2>
                <div className="rounded-lg border divide-y">
                  <div className="px-4 py-3 flex items-start justify-between gap-3">
                    <p className="text-xs text-muted-foreground">
                      Choose Watch all, select specific mailboxes, or leave
                      every mailbox unchecked to pause mailbox watching.
                    </p>
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <input
                          ...
                          checked={watchingAll}
                          onChange={(e) =>
                            setWatchedIds(e.target.checked ? null : [])
                          }
                      ...
                      {personalMailboxes.length > 0 && (
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={showSharedMailboxes}
                            onChange={(e) =>
                              setShowSharedMailboxes(e.target.checked)
                            }
                            className="size-3.5 rounded border-input accent-[#2563eb]"
                          />
                          <span className="text-[12.5px] text-muted-foreground">
                            Include personal Inbox mailboxes
                          </span>
                        </label>
                      )}
                    </div>
                  ...
                  {visibleMailboxes.length === 0 ? (
                    <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                      No support inbox connected yet. Connect one above to get
                      started.
                    </div>
                  ) : (
                    visibleMailboxes.map((conn) => {
                      const checked = watchingAll
                        ? false
                        : (watchedIds?.includes(conn.id) ?? false)
                      return (
                        <label
                          key={conn.id}
                          className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-muted/30 transition"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleMailbox(conn.id)}
              ...
              <section className="mb-8">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Use a dedicated Twilio number for support texts. Texts to this
                  number are drafted by Noli and follow the reply mode in
                  Settings. Connect your Twilio account in Settings first. Use a
                  number that is different from the one your Inbox uses, so
                  support texts and inbox texts stay separate.
                </p>
                <div className="rounded-lg border">
                  <div className="px-4 py-3 space-y-2">
                    ...
                    {twilioNumber ? (
                      csSmsNumber.trim() === '' ? (
                        <p className="text-[12.5px] text-muted-foreground">
                          Your connected Twilio number is {twilioNumber}. Enter
                          a different number to dedicate to support, then set
                          its inbound webhook below.
                        </p>
                      ) : (
                        <p className="text-[12.5px] text-muted-foreground">
                          Enter the Twilio number you want to use only for
                          support. It must be different from your Inbox number (
                          {twilioNumber}).
                        </p>
                      )
                    ) : (
                      <p className="text-[12.5px] text-muted-foreground">
                        No Twilio account connected yet. Connect Twilio in
                        Settings, then enter a dedicated support number here.
                      </p>
                    )}
                    <div className="rounded-md bg-muted/40 border px-3 py-2 mt-1">
                      <p className="text-[12.5px] font-medium text-muted-foreground mb-1">
                        In Twilio, set this number&apos;s inbound message
                        webhook to:
                      </p>
                      <code className="text-xs break-all">
                        https://crm.noliai.com/api/sms/webhook
                      </code>
                      <p className="text-[12.5px] text-muted-foreground mt-1.5">
                        Method POST. This is the same webhook your Inbox uses,
                        so the number you choose here must be a separate number
                        from your Inbox number.
                      </p>
                    </div>
                    <div className="pt-1">
                      <TwilioSmsGuide />
