...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (transcript.length < 20) {
      return NextResponse.json(
        { ok: false, error: 'Say (or type) a bit more about the call first.' },
        { status: 400 },
      )
    }
    if (transcript.length > 8000) {
      return NextResponse.json(
        {
          ok: false,
          error: 'Debrief is too long. Keep it under a few minutes of talking.',
        },
        { status: 400 },
      )
    }
    ...
    const knex = (
      (await createRequestContainer()).resolve('em') as EntityManager
    ).getKnex()
    ...
    const clientToday =
      typeof body.today === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(body.today)
        ? body.today
        : null
    const todayDate = clientToday
      ? new Date(`${clientToday}T12:00:00`)
      : new Date()
...
${transcript}`
    ...
    if (!text)
      return NextResponse.json(
        { ok: false, error: 'Could not process the debrief. Try again.' },
        { status: 502 },
      )
    ...
    for (const t of (plan.tasks ?? []).slice(0, 6)) {
      ...
      try {
        await knex('tasks').insert({
          ...
          due_date:
            t.dueDate && /^\d{4}-\d{2}-\d{2}$/.test(t.dueDate)
              ? new Date(t.dueDate)
              : null,
        })
        created.tasks++
      } catch {
        /* one bad row must not sink the whole debrief */
      }
    }
    ...
    for (const c of (plan.commitments ?? []).slice(0, 6)) {
      ...
      try {
        ...
        /* commitments table missing — skip */
      }
    }
    ...
    const emailDraft =
      plan.emailDraft &&
      typeof plan.emailDraft.body === 'string' &&
      plan.emailDraft.body.trim()
        ? {
            subject: String(plan.emailDraft.subject ?? 'Following up').slice(
              0,
              200,
            ),
            body: String(plan.emailDraft.body).slice(0, 4000),
          }
        : null
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      summary:
        'Voice debrief: transcript to note + tasks + commitments + draft',
    },
  },
}
